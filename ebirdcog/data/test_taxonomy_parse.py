...
common, scientific, code, short = taxonomy_parse.taxonomy_parse(fn)
...
banding_mapping = taxonomy_parse.common_name_to_banding(csv_filename)
banding_mapping_all = taxonomy_parse.common_name_to_banding(csv_filename, True)
...
def test_banding_included(name, banding_code):
    assert banding_mapping[name] == banding_code
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Streptopelia decaocto", ["ECDO", "STDE"]),
        ("Calocitta colliei", ["BTMJ", "CACO"]),
        ("Fregetta tropica", ["BBSP", "FRTR"]),
        ("Setophaga virens", ["BTGW", "SEVI"]),
        ("Knipolegus aterrimus", ["WWBT", "KNAT"]),
